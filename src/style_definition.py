...
def get_user_widget_size(widget_name, resolution, dimension, default):
    """
    Get user-defined widget size or return default.

    Args:
        widget_name: Widget name (battery, control, network, calendar, shortcuts, disk, wallpaper, style_settings)
        resolution: Screen width as string or int (1920, 2560, 3440, 3840)
    ...
    Returns:
        int: Size in pixels
    """
    widget_sizes = USER_SETTINGS.get("widget_sizes", {})
    res_sizes = widget_sizes.get(str(resolution), {})
    widget_data = res_sizes.get(widget_name, {})
    return int(widget_data.get(dimension, default))

def resolution_bracket(monitor_id):
    """Which of the four widths a screen's settings are stored under.

    user-settings.json keys both the scaling factors and the widget sizes
    by screen width, and only these four keys exist. A screen of any
    other width takes the first bracket it fits in, so a 2160-wide one
    reads the 2560 settings rather than none at all.

    The width is the one the screen has as it STANDS: monitors.py turns a
    rotated panel's mode round for us, so a 4K screen on its side lands in
    the 2560 bracket, where it belongs, instead of the 3840 one.

    A monitor_id no screen fills - MON3 on a two-screen desk, or anything
    at all when there is no compositor to ask - falls back to
    FALLBACK_RESOLUTION rather than borrowing a neighbour's size.
    """
    width = MONITOR_RESOLUTIONS.get(monitor_id, FALLBACK_RESOLUTION)[0]
...
def get_widget_size_for_monitor(widget_name, monitor_id, dimension, default):
    """
    Get widget size for a specific monitor based on its resolution.

    Args:
        widget_name: Widget name
        monitor_id: The screen's place on the desk, counted from the left
        dimension: "width" or "height"
        default: Default size value

    Returns:
        str: Size with 'px' suffix
    """
    size = get_user_widget_size(
        widget_name, resolution_bracket(monitor_id), dimension, default)
    return f"{size}px"

def get_widget_scroll_for_monitor(widget_name, monitor_id, default):
    """
    Get widget scroll height for a specific monitor (without px suffix).
    """
    res_key = resolution_bracket(monitor_id)

    # Scroll height = widget height minus offset for header/footer/actions
    # Different widgets have different header/footer sizes
    height = get_user_widget_size(widget_name, res_key, "height", default)

    # Offset based on widget type (header + footer + padding)
    offsets = {
        "style_settings": 120,  # Header + tabs + actions
        "disk": 200,            # Header + circular progress + actions
        "shortcuts": 80,        # Header + footer
        "wallpaper": 180,       # Header + current wallpaper + actions
        "network": 80,          # Header + footer
        "calendar": 60,         # Header only
        "battery": 80,          # Header + actions
        "control": 80,          # Header + actions
    }
    offset = offsets.get(widget_name, 100)
    scroll_height = max(height - offset, 150)  # minimum 150px

    return str(scroll_height)
...
#     is gone rather than wired up: what a widget is HIGH already comes
#     from widget_sizes.<width>.<widget>.height, in pixels, per bracket,
#     and multiplying that by a second per-bracket factor would mean two
#     controls for one number. See user_settings.migrate_scaling for what
#     happens to a height a user had already stored.
#
...
# {{STYLE_EWW_WINDOW_DISK_MON2}} would resolve on a three-screen desk and
...
def _per_screen(values):
    ...
    value that had to change had to change in five places. The same
    argument, and the same fix, as the widget widths further down - the
    comment beside _WIDGET_WINDOW_WIDTHS records that those were five
    copies too, and this is the rest of them.
...
_FIXED_STYLE_VARIABLES = {
    ...
    **_per_screen({
    ...
    # Scroll height = widget height - header/footer offset
    # ============================================================================
    "STYLE_EWW_SCROLL_DISK": str(max(get_user_widget_size("disk", "1920", "height", 320) - 200, 150)),
    "STYLE_EWW_SCROLL_SHORTCUTS": str(max(get_user_widget_size("shortcuts", "1920", "height", 520) - 80, 150)),
    "STYLE_EWW_SCROLL_WALLPAPER": str(max(get_user_widget_size("wallpaper", "1920", "height", 280) - 180, 100)),
    "STYLE_EWW_SCROLL_STYLE_SETTINGS": str(max(get_user_widget_size("style_settings", "1920", "height", 550) - 120, 150)),
    "STYLE_EWW_SCROLL_STYLE_SETTINGS_WIDGETS": str(max(get_user_widget_size("style_settings", "1920", "height", 550) - 170, 150)),
    "STYLE_EWW_DISK_RING_SIZE": "150",
    "STYLE_EWW_DISK_RING_THICKNESS": "12",
    "STYLE_EWW_MIN_WIDTH_KEY": "155px",
    "STYLE_EWW_MIN_WIDTH_COL": "260px",
...
GLASS_LAYERS = (
    ...
    "notification-center",
    "network",
    # Seit dem 17.08.2026, mit dem Fenster dazu. Es kommt aus derselben
    # Fabrik wie das Netzfenster daneben und traegt deshalb dieselbe
    # Platte - siehe GLASS_PLATES.
    "bluetooth",
    "calendar",
    "shortcuts",
    "battery",
...
# Widget -> default WIDTH of its window, and widget -> default HEIGHT of
# its scrolling area. The placeholder is the widget's name in capitals,
# which is why one table can drive ten placeholders per screen: the five
# blocks these replaced were the same ten lines copied five times, and
# every value that had to change had to change in five places.
_WIDGET_WINDOW_WIDTHS = {
    "battery": 360,
    "control": 380,
    "network": 380,
    "calendar": 420,
...
def _monitor_style_variables():
    ...
    for index in range(MONITOR_SLOTS):
        ...
        for widget, default in _WIDGET_WINDOW_WIDTHS.items():
            variables[f"STYLE_EWW_WINDOW_{widget.upper()}_MON{index}"] = (
                get_widget_size_for_monitor(widget, index, "width", default))
        for widget, default in _WIDGET_SCROLL_HEIGHTS.items():
            variables[f"STYLE_EWW_SCROLL_{widget.upper()}_MON{index}"] = (
                get_widget_scroll_for_monitor(widget, index, default))
    return variables
